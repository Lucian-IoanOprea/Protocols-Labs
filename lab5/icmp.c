...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	for (int i = 0; i < count || count == -1; ++i) {
		...
		*/
		...
		memcpy(&ip_hdr->daddr, &ip_addr.s_addr, sizeof(icmp_hdr));
		...
		*/
		...
		printf("64 bytes from %s: icmp_seq=%hu ttl=%u time=%.1f ms\n",
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	while(i <= max_hops) {
		...
		if (ricmp_hdr->type == ICMP_TIME_EXCEEDED && ricmp_hdr->code == 0) {
			struct in_addr temp;
			temp.s_addr = rip_hdr->saddr;
			printf("%s\n", inet_ntoa(temp));
		}
