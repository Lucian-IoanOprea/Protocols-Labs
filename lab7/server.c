...
void run_echo_server(int listenfd)
{
    ...
    listen(listenfd, 5);
	printf("Se accepta o singura conexiun");
    // TODO 5: Acceptati o conexiune


    connfd = accept(listenfd, NULL, NULL);
    send(connfd, "1", strlen("1") + 1, 0);
...
void run_chat_server(int listenfd)
{
	...
	printf("Se accepta conexiunile clientilor");
...
int main(int argc, char* argv[])
{
    ...
    int ok = 1;
    if (setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, &ok, sizeof(int)) == -1) {
        perror("setsocketopt");
        exit(1);
    }
